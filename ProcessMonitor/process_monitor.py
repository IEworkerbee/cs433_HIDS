...
from ..Network.config import CPU_PERCENTAGE
...
def get_proc_cpu_use(pid, msg_queue):
    try:
        proc = psutil.Process(pid)
...
def spawn_info_threads(pid, msg_queue):
    threads = []
    ...
    threads.append(threading.Thread(target=get_proc_cpu_use, args=(pid, msg_queue), daemon=True))
...
def _process_monitor(msg_queue, eventflag: threading.Event):
            ...
            try:
                ...
                if pid not in EXCLUDED_PIDS:
                    ...
                    if pid not in pids_prev:
                        ...
                        info_threads = spawn_info_threads(pid, msg_queue)
    ...
    dump_loc.write("\nAll done!\n")
    dump_loc.write(f"Elapsed time: {datetime.now().timestamp() - start_time}")
    dump_loc.close()

def cpu_monitor(msg_queue: Queue, eventflag: threading.Event):
    proc_mon = threading.Thread(target=_process_monitor, args=(msg_queue, eventflag,))
    proc_mon.start()
    proc_mon.join()